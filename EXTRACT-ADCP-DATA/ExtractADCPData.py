...
class ADCPDataExtractor:
   ...
   def extract_one_day_data(self, target_date, param, nrange= None):
        #if(nrange== None):
        #    r        = df.range.data
        #else:
        ...
        if(param=='U_mag'):
                ...
                try:
                    ...
                    print("The Data seems not to exist for the certain data in this day: %s "%(target_date))
        elif(param=='u'):
            ...
            #Loop over the list of the time
                ...
                try:
                    data  = np.nanmean(self.data[param].sel(time = target_date), axis =0)
   ...
   def extract_several_days_data(self, target_date, param, nrange= None):
        ############################################
        ...
        if(param=='U_mag'):
            ...
            try:
                ...
                try:
                    ...
                    data  = np.concatenate([np.nanmean(self.data[param].sel(time = ti), axis=0) for ti in target_date])
        ...
        #    try:
        #        data  = np.concatenate([np.nanmean(self.data.velds.param.sel(time = ti), axis =0) for ti in target_date])
        #        time  = np.concatenate([self.data.velds.param.sel(time = ti)['time'] for ti in target_date])
        #    except:
        #        print("The Data seems not to exist for the certain data in the list Time :  ")
        #        print(' , '.join(target_date))
        ###################################
#   def extract_data(self, target_date):
#        ############################################
#                P  = np.nanmean(self.data.velds.U_mag.sel(time = target_date), axis =0)
#                #P  = np.nanmean(self.data.velds['U_mag'].sel(time = target_date), axis =0)
#                T  = self.data.velds.U_mag.sel(time = target_date)['time'] 
#                return P
...
if __name__ == "__main__":
    ...
    #P, T = extractor.extract_one_day_data(target_date, "U_mag", nrange=15)
    #P, T = extractor.extract_one_day_data(target_date, "U_dir", nrange=15)
    #P, T = extractor.extract_data(target_date, "w", nrange=15)
    #P, T  = extractor.extract_several_days_data(target_date, "U_dir", nrange=15)
    P, T  = extractor.extract_several_days_data(target_date, "U_mag", nrange=15)
    #print(day_data)
    print(P)
