...
#tadnew, d_ampnew =  inter1DD(tadd,  d_amp, ts)

#wamba
tsnew, Powernew  =  inter1DD(ts,  Power, tadd)

# Convert to hours
#tsnew = tsnew * 24  # Convert days to hours
fig, ax    = plt.subplots(1, 1, sharex = False, figsize = (12,11))

#tsnew = mdates.num2date(tsnew) 

#plot the Seismic energy
ax.plot(tadd, Powernew, color='k', label='Seismic Energy') 
ax.plot(tadd, Powernew, color='k', lw= 14, alpha =0.3 ) 

# Create a second y-axis sharing the same x-axis 
ax2 = ax.twinx() 
#Plot the back scatter
ax2.plot(tadd, d_amp, color='r', label='Backscatter') 
ax2.plot(tadd, d_amp, color='r', lw=14, alpha = 0.3) 
...
figname = "Backscatter_BS_%s.png"%(Date_of_Day)
#figname = "Backscatter_BS_%s_TEST.png"%(Date_of_Day)
#
ax.set_title('Seismic energy and backscatter on %s' %(Date_of_Day), loc ='center', fontsize=fsize, pad =pad)
#Make the grid of the axis
ax.grid(visible = True, axis = "both", alpha = 0.9)
#Add the legend
#ax.legend(loc="upper left")
#Set label
ax.set_ylabel("Seismic energy (dB)", color ='k', labelpad = pad, fontsize=fsize)
###########################################
ax2.set_ylabel('Backscatter (counts)', color='r', labelpad = pad, fontsize=fsize)
...
ax.set_xlabel("Time (hour:minute) on %s"%(Date_of_Day),fontsize=fsize, labelpad =pad)
ax.xaxis.set_major_formatter(mdates.DateFormatter("%H:%M"))
#Add the legend
ax.legend(loc="upper left", fontsize=fsize)
ax2.legend(loc="upper right", fontsize=fsize)
##################################################
ax.tick_params(axis ='both', labelsize= lsize)
