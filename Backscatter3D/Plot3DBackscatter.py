...
def Plot3DBackscatter(ax, df,TARGETDATE, height_adcp, depth_idx, ADCP_DOWN=False, VERTICAL_SLIDES =None):
    ...
    if(VERTICAL_SLIDES):
        ...
        for d in range(n_depth):
            ...
            ax.plot_surface(T, B, depth_layer, rstride=10, cstride=1, facecolors=cm.RdYlBu_r(slice_data / slice_data.max()),
                            shade=False, alpha=0.7, edgecolor='none')
        
        #Highlighted slice
        highlight_data = backscat[:, depth_idx, :]
        #ax.plot_surface(B, T, D, rstride=10, cstride=1, facecolors=cm.RdYlBu_r(highlight_data / highlight_data.max()),
    ...
    #Plot the horizontal slice
            ...
            for d in range(n_depth):
                slice_data = backscat[:, d, :]
                depth_layer = np.full_like(T, depths[d])
                ax.plot_surface(T, depth_layer, B, rstride=10, cstride=1, facecolors=cm.RdYlBu_r(slice_data / slice_data.max()),
                                shade=False, alpha=0.7, edgecolor='none')
            #Highlighted slice
            highlight_data = backscat[:, depth_idx, :]
            ax.plot_surface(T, D, B, rstride=10, cstride=1, facecolors=cm.plasma(highlight_data / highlight_data.max()),
            #ax.plot_surface(T, D, B, rstride=10, cstride=1, facecolors=cm.RdYlBu_r(highlight_data / highlight_data.max()),
                            shade=False, alpha=0.9, edgecolor='k')
...
if(VERTICAL_SLIDES):
    ...
    ax.set_ylabel("Time (hour:minute)",fontsize=10, labelpad =12)
    ax.yaxis.set_major_formatter(mdate.DateFormatter("%H:%M"))
    ##################################################
    ax.tick_params(axis ='x', labelsize= lsize, pad=1.5)
    ax.tick_params(axis ='y', labelsize= 9, labelrotation=310,zorder=12, pad=1.2)
...
plt.colorbar(cm.ScalarMappable(cmap='RdYlBu_r'), ax=ax, label='Backscatter Intensity', pad =0.12, extend ='both', shrink=0.5)
